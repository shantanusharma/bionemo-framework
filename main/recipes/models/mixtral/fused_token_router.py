...
class FusedTokenRouter:
    ...
    def dispatch(
        ...
        # Step 1: Fused all-to-all dispatch (DeepEP)
        recv_x, recv_indices, recv_probs, tokens_per_expert, deepep_handle = fused_dispatch(
            hidden_states,
            selected_experts,
            routing_weights.float(),  # DeepEP requires float32 probs
            self.num_experts,
            self._ep_group,
        )
    ...
    def combine(self, expert_output: torch.Tensor, handle: _FusedHandle) -> torch.Tensor:
        ...
        """
        # Step 1: Unpermute expert output and apply routing weights
        ...
        # Step 2: Fused all-to-all combine (reverse dispatch)
