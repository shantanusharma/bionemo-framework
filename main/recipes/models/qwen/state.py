...
# This file is copied from: models/esm2/state.py
...
def apply_transforms(
    ...
    Args:
        ...
        cast_dtype: case the output state dict to a certain precision.
    ...
    for key, val in mapping.items():
        logger.debug(f"Mapping {key} -> {val}")
        ctx = StateDictTransform(key, val)(ctx)

    for transform in transforms:
        logger.debug(f"Transforming {transform.source_key} -> {transform.target_key}")
        ctx = transform(ctx)
    ...
    if cast_dtype:
        logger.info(f"Casting model to {cast_dtype}...")
...
class StateDictTransform(Generic[F]):
    """A transformation class for state dictionaries.

    Allows for flexible key matching and transformation of values between source and target state dictionaries.
    ...
    def __init__(
        ...
        """Initialize the StateDictTransform."""
        ...
        if isinstance(source_key, (dict, tuple)):
            ...
            for layer_names_group in zip(*(source_matches + target_matches)):
                ...
                if isinstance(layer_names_group[0], str):
                    layer_names_group = [[x] for x in layer_names_group]  # noqa: PLW2901
                ...
                logger.debug(f"Matched (transform)! {layer_names_group=}")
                matched = True
        else:
            ...
            if multiple_sources:
                for target_index, target_match in np.ndenumerate(target_matches):
                    ...
                    logger.debug(f"Matched (multi source)! {target_match=} {source_match=}")
                    matched = True
            else:
                for source_index, source_match in np.ndenumerate(source_matches):
                    ...
                    logger.debug(f"Matched (single source)! {target_match=} {source_match=}")
                    matched = True
        if not matched:
...
def state_transform(
    ...
    """Create a StateDictTransform instance with specified source and target keys, and a transformation function.
...
def extract_dtypes(ckpt):
    """Extract dtype from the input iterator.
