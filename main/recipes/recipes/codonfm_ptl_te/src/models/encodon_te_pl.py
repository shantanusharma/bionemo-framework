...
from src.models.components.encodon_te import EnCodonTE  # noqa: E402
from src.models.utils import construct_pretrained_config, get_decay_parameter_names  # noqa: E402


class EncodonTEPL(LightningModule):
    ...
    def configure_model(self, state_dict: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.model is None:
            self.model = EnCodonTE(self.pretrained_config)
    ...
    def forward(self, batch_data, return_hidden_states: bool = False):
        ...
        attention_mask = batch_data[MetadataFields.ATTENTION_MASK] if self.attn_input_format == "bshd" else None
        output = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            return_hidden_states=return_hidden_states,
            cu_seq_lens_q=batch_data[MetadataFields.CU_SEQ_LENS_Q]
            if MetadataFields.CU_SEQ_LENS_Q in batch_data
            else None,
    ...
    def model_step(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.hparams.use_downstream_head:
            ...
            projected_states = self.model.cross_attention_input_proj(hidden_states)
