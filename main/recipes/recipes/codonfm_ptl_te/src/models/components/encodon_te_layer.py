...
from transformer_engine.pytorch.module import LayerNorm, LayerNormLinear, RMSNorm
...
# Modified MultiHeadAttention for Encodon, which adds an extra post-attention, pre-projection LayerNorm.
# This LayerNorm will be fused to the linear projection.
from .encodon_te_mha import EncodonTEMultiheadAttention
...
class EncodonTELayer(torch.nn.Module):
    r"""TransformerLayer is made up of an attention block and a feedforward network (MLP).
    ...
    def __init__(  # noqa: D107
        ...
        if parallel_attention_mlp:
            ...
            assert not self.apply_residual_connection_post_layernorm, (
                "parallel_attention and apply_residual_connection_post_layernorm not supported simultaneously."
            )
            assert not self.output_layernorm, "parallel_attention and output_layernorm not supported simultaneously"
        ...
        self.self_attention = EncodonTEMultiheadAttention(
            *attention_args,
            **common_attention_kwargs,
        ...
        if layer_type == "decoder":
            self.inter_attention = EncodonTEMultiheadAttention(
                *attention_args,
                **common_attention_kwargs,
        ...
        # MLP: LayerNorm -> Linear -> Activation -> LayerNorm -> Linear
        self.mlp_fc1 = LayerNormLinear(
            hidden_size,
            ffn_hidden_size,
            ...
            normalization=normalization,
            ub_name="mlp_fc1",
            name=name + ".mlp_fc1" if name is not None else None,
        )
        self.mlp_activation = ACT2FN[activation]
        self.mlp_fc2 = LayerNormLinear(
            ffn_hidden_size,
            hidden_size,
            eps=layernorm_epsilon,
            fuse_wgrad_accumulation=fuse_wgrad_accumulation,
            ...
            normalization=normalization,
            ub_name="mlp_fc2",
            name=name + ".mlp_fc2" if name is not None else None,
        )
    ...
    def forward(
        ...
        assert enc_dec_attn_mask_type in AttnMaskTypes, (
        ...
        if self.sequence_parallel and self.seq_length is not None:
            assert hidden_states.shape[0] == self.seq_length // self.tp_size, (
                "Sequence dimension must be split across TP group when using sequence parallel."
            )

        if ("padding" in self_attn_mask_type or self_attn_mask_type == "arbitrary") and attention_mask is not None:
            assert all(attention_mask[i].dtype == torch.bool for i in range(len(attention_mask))), (
        ...
        if (
            ...
            assert all(enc_dec_attn_mask[i].dtype == torch.bool for i in range(len(enc_dec_attn_mask))), (
        ...
        if self.apply_residual_connection_post_layernorm and not self.output_layernorm:
            ...
            hidden_states = self._bias_dropout_add(attention_output, attention_bias, residual, self.drop_path)
        elif not self.parallel_attention_mlp:
            attention_output, attention_bias = self_attention_outputs
            hidden_states = self._bias_dropout_add(attention_output, attention_bias, hidden_states, self.drop_path)
        ...
        # MLP.
        mlp_fc1_output = self.mlp_fc1(hidden_states)
        mlp_act_output = self.mlp_activation(mlp_fc1_output)
        mlp_fc2_output = self.mlp_fc2(mlp_act_output)
        mlp_outputs = (
            torch.nn.functional.dropout(mlp_fc2_output[0], p=self.hidden_dropout, training=self.training),
            *mlp_fc2_output[1:],
        )
        ...
        if self.apply_residual_connection_post_layernorm:
            ...
            output = self._bias_dropout_add(self_attention_outputs, mlp_outputs, hidden_states, self.drop_path)
        else:
    ...
    def _bias_dropout_add(self, hidden_state, bias, residual, drop_path=None):
        if drop_path is None and bias is not None and bias.numel() != 0:
            ...
            out = torch.nn.functional.dropout(hidden_state, p=self.hidden_dropout, training=self.training)
