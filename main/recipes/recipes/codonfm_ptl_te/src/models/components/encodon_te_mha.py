...
from transformer_engine.pytorch.module import LayerNorm, LayerNormLinear, Linear, RMSNorm
...
class EncodonTEMultiheadAttention(torch.nn.Module):
    r"""Multi-head Attention (MHA), including Query,
    ...
    def _create_qk_norm_modules(
        ...
        raise ValueError(
            f"Unsupported QK norm type: {qk_norm_type}. Supported types: ['L2Normalization', 'RMSNorm', 'LayerNorm']"
        )
    ...
    def set_context_parallel_group(
        ...
        if isinstance(cp_group, dist_group_type):
            ...
            assert cp_comm_type == "a2a+p2p", "Only cp_comm_type of a2a+p2p requires hierarchical CP groups!"
            assert len(cp_group) == 2, "cp_comm_type = a2a+p2p requires cp_group = [a2a_cp_group, p2p_cp_group]!"
    ...
    def forward(
        ...
        if self.attention_type == "self":
            ...
            if self.qkv_weight_interleaved:
                ...
                new_tensor_shape = mixed_x_layer.size()[:-1] + (  # noqa: RUF005
            ...
            if self.qkv_weight_interleaved:
                ...
                new_tensor_shape = mixed_kv_layer.size()[:-1] + (  # noqa: RUF005
                    self.num_gqa_groups_per_partition,
            ...
            if self.qkv_format == "thd":
                key_layer, value_layer = (
                    x.reshape(x.size(0), -1, self.hidden_size_per_attention_head) for x in (key_layer, value_layer)
                )
            else:
                # key, value: -> [sq, b, ng, hn]
                key_layer, value_layer = (
            ...
            # [sq, b, hp] --> [sq, b, np, hn]
            new_tensor_shape = query_layer.size()[:-1] + (  # noqa: RUF005
        ...
        if rotary_pos_emb is not None:
            assert not isinstance(query_layer, Float8Tensor) and not isinstance(key_layer, Float8Tensor), (
