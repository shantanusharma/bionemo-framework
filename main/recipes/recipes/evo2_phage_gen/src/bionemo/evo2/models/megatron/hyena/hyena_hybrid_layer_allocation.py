...
# This file is copied from: recipes/evo2_megatron/src/bionemo/evo2/models/megatron/hyena/hyena_hybrid_layer_allocation.py
...
class Symbols:
    ...
    HYENA = "H"
    ATTENTION = "*"
    VALID: ClassVar[set[str]] = {HYENA_SHORT, HYENA_MEDIUM, HYENA, ATTENTION}
...
def _allocate_override(total_layers_count: int, override_pattern: str) -> list:
    ...
    for layer_type in layer_type_list:
        if layer_type not in Symbols.VALID:
            raise ValueError(f"In hybrid override pattern, '{layer_type}' is not one of {Symbols.VALID}")
...
def allocate_layers(
    ...
    log_single_rank(
        logger,
        logging.INFO,
        f"{actual_hyena_short_layers_count} heyna_short_conv layers in {total_layers_count} total layers.",
    )
    log_single_rank(
        logger,
        logging.INFO,
        f"{actual_hyena_medium_layers_count} heyna_medium_conv layers in {total_layers_count} total layers.",
    )
    log_single_rank(
        logger,
        logging.INFO,
        f"{actual_hyena_layers_count} heyna layers in {total_layers_count} total layers.",
    )
    log_single_rank(
        logger,
        logging.INFO,
