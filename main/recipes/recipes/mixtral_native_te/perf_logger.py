    ...
    def log_step(
        ...
        with torch.no_grad():
            ...
            if step % self.logging_frequency == 0 and step > 0:
                ...
                # max_memory_allocated tracks the true peak (during fwd/bwd when all activations are
                # live), not just the resting allocation at log time; reset each window so the peak is
                # measured fresh over the window. memory_allocated is the resting (post-step) value.
                peak_allocated = torch.cuda.max_memory_allocated() / (1024**3)
                resting_allocated = torch.cuda.memory_allocated() / (1024**3)
                self.metrics["train/gpu_memory_allocated_max_gb"].update(peak_allocated)
                self.metrics["train/gpu_memory_allocated_mean_gb"].update(resting_allocated)
    ...
    def __init__(
        ...
        if self.running_under_nsys:
            ...
            logger.warning(
                ...
                "--capture-range-end=stop python train_fsdp2_ep.py profiler.enabled=true"
            )
