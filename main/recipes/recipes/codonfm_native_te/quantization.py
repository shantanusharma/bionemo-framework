...
def generate_layer_regex(layer_numbers: list[int] | None) -> str:
    ...
    if not layer_numbers:
        return r"encoder\.layers\.DISABLED_NO_LAYERS_SPECIFIED"
    layer_pattern = "|".join(str(n) for n in sorted(layer_numbers))
    return rf"encoder\.layers\.({layer_pattern})\..*(layernorm_qkv|proj|fc1|fc2)"
...
def resolve_layer_precision(
    ...
    Raises:
        ValueError: If both formats are enabled with no layer lists, if layer lists overlap,
            or if layer entries are out of range.
    """
    ...
    for name, layers in [("fp8_layers", fp8_layers), ("fp4_layers", fp4_layers)]:
        if layers is not None and len(layers) > 0:
            ...
            if invalid:
                raise ValueError(
                    f"{name} contains out-of-range or non-integer entries: {invalid}. "
                    f"Must be integers in 1..{num_layers}."
                )
